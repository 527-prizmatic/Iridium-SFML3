...
namespace iridium {
	class ApplicationWindow: public iridium::RenderTarget {
	public:
		ApplicationWindow(iridium::Vector _size);
		...
		void setSize(iridium::Vector _size);
		...
		/// @attention Attempting to call this function while the window is in an invalid state will throw an exception.
		void render(iridium::render::Shape& _shape);

		/// @attention Attempting to call this function while the window is in an invalid state will throw an exception.
		void render(iridium::RenderTarget& _render_target);
		...
		bool isValid() {
			return this->m_renderWindow != nullptr && this->renderTexture_ != nullptr && this->rect_ != nullptr;
		}

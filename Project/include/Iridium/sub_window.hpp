...
namespace iridium {
	class SubWindow: public iridium::RenderTarget {
	public:
		SubWindow(iridium::Vector size);
		...
		void setSize(iridium::Vector size);
		...
		/// @attention Attempting to call this function while the buffer is in an invalid state will throw an exception.
		void render(iridium::render::Shape& shape);

		/// @brief Draws the contents of another render target into this one.
		/// @attention Attempting to call this function while the buffer is in an invalid state will throw an exception.
		void render(iridium::RenderTarget& renderTarget);
		...
		void flushToTarget(iridium::RenderTarget& renderTarget);
		...
		void setPosition(iridium::Vector pos) { position_ = pos; }
		iridium::Vector getPosition() const { return position_; }

	private:
		std::unique_ptr<sf::RenderTexture> renderTexture_;
		std::unique_ptr<iridium::render::Quad> rect_; ///< Rectangle shape used for rendering the subwindow upon calling flushToTarget()
		std::unique_ptr<iridium::render::Rectangle> frame_; ///< Rectangle shape used to render the frame
		iridium::Vector position_; ///< Where to draw the subwindow relative to its parent
		...
		void allocateResources(iridium::Vector size);

		void configureRect(iridium::Vector size);
		void configureFrame(iridium::Vector size);
	};
}
